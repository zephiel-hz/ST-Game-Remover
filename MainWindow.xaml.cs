...
namespace SteamPluginManager
{
    public partial class MainWindow : Window, INotifyPropertyChanged
    {
        ...
        private static readonly string Password = "a-very-secure-password-for-steam-plugin-manager"; // Replace with a secure password
        private const int SaltSize = 16;
        
        // Fields to store original effects for performance optimization
        private Effect? _headerBorderEffect;
        ...
        private void EncryptStream(MemoryStream inStream, string outputFile)
        {
            ...
            using (var aes = Aes.Create())
            {
                aes.KeySize = 256;
                aes.GenerateIV();
                var key = new Rfc2898DeriveBytes(Password, salt, 10000, HashAlgorithmName.SHA256).GetBytes(aes.KeySize / 8);
                aes.Key = key;
        ...
        private void SaveLanguagePreference(string language)
        {
            try
            {
                ...
                var appFolder = System.IO.Path.Combine(appDataPath, "STGameRemover");
        ...
        public MainWindow()
        {
            try
            {
                ...
                _headerBorderEffect = HeaderBorder.Effect;
                _gameListBorderEffect = GameListBorder.Effect;
                _gameDetailsBorderEffect = GameDetailsBorder.Effect;
                _logBorderEffect = LogBorder.Effect;
        ...
        private void SetShadowsEnabled(bool enabled)
        {
            try
            {
                if (enabled)
                {
                    ...
                    HeaderBorder.Effect = _headerBorderEffect;
        ...
        private int DeleteManifestFile(string appId, string manifestId)
        {
            try
            {
                ...
                string depotCachePath = System.IO.Path.Combine(steamPath, "config", "depotcache", manifestFileName);

                if (File.Exists(depotCachePath))
                {
                    File.Delete(depotCachePath);
                    Log($"Deleted: {manifestFileName}");
                    return 1;
                }
            }
            catch (Exception ex)
        ...
        private void RefreshGameList()
        {
            ...
            foreach (var file in Directory.GetFiles(pluginPath, "*.lua"))
            {
                try
                {
                    ...
                    {
                        // Coba dapatkan nama dari cache dulu
                        string? cachedName = CacheManager.GetGameName(appId);
            ...
            ApplyFilterAndSort();
            Log($"Loaded {Games.Count} game(s). Cache: {CacheManager.GetCacheSize()} items");
        ...
        private void RestoreBackup_Click(object sender, RoutedEventArgs e)
        {
            ...
            try
            {
                using (var memoryStream = DecryptStream(openFileDialog.FileName))
                {
                    ...
                    string depotCachePath = System.IO.Path.Combine(steamPath, "config", "depotcache");
        ...
        private string? GetGameInstallationPath(int appId)
        {
            try
            {
                foreach (var library in EnumerateSteamLibraries())
                {
                    ...
                    if (File.Exists(manifest)) return library;
                }
            }
            catch { }
            return null;
            ...
            try
            {
                    ...
                    app.TryGetProperty("data", out var data))
                {
                             ...
                             screenshots.GetArrayLength() > 0)
                    {
                        ...
                        catch { /* Silently handle screenshot extraction errors */ }
                    }
                        ...
                        publishers.GetArrayLength() > 0)
                    {
                        ...
                        catch { /* Silently handle publisher extraction errors */ }
                    }
        ...
        private async Task CheckForUpdatesOnStartupAsync()
        {
            try
            {
                ...
                // Only show popup if there's an update available
            ...
            {
                // Silent fail - don't show error popup on startup
                Logger.Log($"Auto-check for updates failed: {ex.Message}");
            }
        }
                    ...
                    (current, total) =>
                    {
                        Dispatcher.Invoke(() =>
                        {
                            if (total > 0)
                            {
                                ...
                                // Progress dapat ditambahkan di sini jika perlu
                            }
                        });
                    }
                );
